...
export function SyntaxGuide({ onClose }: SyntaxGuideProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-transparent">
        ...
        {categories.map((cat, i) => (
          <div key={i} className="space-y-4">
            <div className="flex items-center gap-2 pb-2 border-b border-zinc-800/50">
              {cat.icon}
              <h3 className="text-xs font-bold uppercase tracking-widest text-zinc-400">{cat.title}</h3>
            </div>
            
            <div className="space-y-3">
              {cat.items.map((item, j) => (
                <div key={j} className="flex flex-col gap-1">
                  <code className="text-xs font-mono bg-[#1a1a1c] border border-zinc-800 px-1.5 py-0.5 rounded-md w-fit text-indigo-400">
                    {item.syntax}
                  </code>
      ...
      <div className="p-6 bg-[#121214] border-t border-zinc-800 shrink-0">
